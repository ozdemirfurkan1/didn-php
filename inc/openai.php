...
// Kısa bir İngilizce metni doğal Türkçeye çevirir. Başarısızsa null döner.
// (Örnek cümle çevirileri için; çağıran taraf önbellekler.)
function openai_translate_tr(string $text): ?string
{
    $text = trim($text);
    if ($text === '' || mb_strlen($text, 'UTF-8') > 400) {
        return null;
    }
    $key = get_openai_key();
    if (!$key) {
    ...
    $payload = json_encode([
        'model'       => get_openai_model(),
        'messages'    => [
            ['role' => 'system', 'content' => 'Sen İngilizceden Türkçeye çeviri yapan profesyonel bir çevirmensin. Verilen İngilizce cümleyi doğal ve akıcı Türkçeye çevir. SADECE çeviriyi döndür; tırnak, açıklama veya ek metin ekleme.'],
